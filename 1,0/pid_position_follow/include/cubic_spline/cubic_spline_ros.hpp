...
#ifndef ROBOMASTER_CUBIC_SPLINE_ROS2_H
#define ROBOMASTER_CUBIC_SPLINE_ROS2_H
...
TrajInfo GenTraj(const nav_msgs::msg::Path& path, nav_msgs::msg::Path& smoothed_path, const float interval = 0.1){
  ...
  tmp_pose.header.stamp = rclcpp::Clock().now();
  for(float i=0; i<cublic_spline.s.back(); i += interval){
    ...
    tmp_pose.pose.orientation = tf2::toMsg(tf2::Quaternion(tf2::Vector3(0, 0, 1), yaw));
    smoothed_path.poses.push_back(tmp_pose);

  }
  return traj_info;
}
#endif //ROBOMASTER_CUBIC_SPLINE_ROS2_H