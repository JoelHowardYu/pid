...
//     return true;
// }
void GetGlobalRobotPose(const std::shared_ptr<tf2_ros::Buffer>& tf_buffer, const std::string& global_frame, geometry_msgs::msg::PoseStamped& robot_pose) {
    try {
        // 获取从 "base_link" 到 "map" 的变换
        geometry_msgs::msg::TransformStamped transform = tf_buffer->lookupTransform(global_frame, "base_link", tf2::TimePointZero);
        ...
        // 如果查找变换失败，打印错误信息
        RCLCPP_ERROR(rclcpp::get_logger("utils"), "%s", ex.what());
    }
}
